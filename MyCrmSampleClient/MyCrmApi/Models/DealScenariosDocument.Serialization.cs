        ...
        internal static DealScenariosDocument DeserializeDealScenariosDocument(JsonElement element)
        {
            ...
            foreach (var property in element.EnumerateObject())
            {
                ...
                if (property.NameEquals("links"))
                {
                    ...
                    links = DealScenariosDocumentLinks.DeserializeDealScenariosDocumentLinks(property.Value);
                    continue;
                }
                if (property.NameEquals("data"))
                {
                    List<DealScenario> array = new List<DealScenario>();
                    foreach (var item in property.Value.EnumerateArray())
                    {
                        array.Add(DealScenario.DeserializeDealScenario(item));
                    }
                    data = array;
                    continue;
                }
            ...
            return new DealScenariosDocument(Optional.ToDictionary(meta), Optional.ToDictionary(jsonApi), links.Value, data, Optional.ToList(included));
        }
    }
}
